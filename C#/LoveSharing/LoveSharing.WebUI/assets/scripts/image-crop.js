var FormImageCrop = function () {
	$(":file").uniform({fileClass: 'uploader'});
	
	
	/*$('.confirm').click(function(){
		//$('.img-crop-view').css('transform','rotate(-90deg)');
		$('.img-crop-view img,.img-crop-view .jcrop-holder>div>img').css('transform','rotate(-90deg)');
		var imgWidth = $('#img-target').width();
		var imgHeight = $('#img-target').height();
		$('.img-crop-view').width(imgHeight);
		$('.img-crop-view').height(imgWidth);
		$('.jcrop-holder').width(imgHeight);
		$('.jcrop-holder').height(imgWidth);
	});*/
	function img_fit(){
		var imgWidth = $('#img-target').width();
		var imgHeight = $('#img-target').height();
		var imgScale = imgWidth / imgHeight;
		var Fwidth = $('.img-crop-view').width();
		var Fheight = $('.img-crop-view').height();
		if(imgHeight > Fheight) {
			imgHeight = Fheight;
			imgWidth = imgHeight * imgScale ;
			$('.img-crop-view img').width(imgWidth);
			$('.img-crop-view img').height(imgHeight);
		}	
		$('.img-crop-view img').css('margin-top',-imgHeight/2 +'px');
		$('.img-crop-view').width(imgWidth+30);
		$('.img-crop-view').height(imgHeight);
	};
	img_fit();
    var demo1 = function() {
        $('#demo1').Jcrop();
	...
	var img_crop = function() {
		...
		var imgWidth = $('#img-target').width();
		var imgHeight = $('#img-target').height();
		var x1 = (imgWidth - imgHeight)/2 ,
			x2 = imgWidth - (imgWidth - imgHeight)/2 
			y1 = 0,
			y2 = imgHeight;
	...
	$('.img-crop-turn-left').click(function(){
		alert("sd");
		$('#img-target').css('transform','rotate(-90deg)');
	});
    return {
