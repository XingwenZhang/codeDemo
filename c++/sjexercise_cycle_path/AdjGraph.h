...
public:
	...
	Edge<EdgeType> FirstEdge(int oneVertex)
	{
		...
		for(int i = 0; i < vertexNum ;i ++)
		{
			if(matrix[oneVertex][i] != INFINITY)                //不为INT_MAX说明有边
			{
				tmpEdge.end = i ;
	...
	Edge<EdgeType> NextEdge(Edge<EdgeType> oneEdge)      //避免跟上面的边重复，因此采用边来作为参数
	{
		...
		{
			if(matrix[oneEdge.start][i] != INFINITY)
			{
	...
	//		if(q == v-1)
	//			i = 0 ;
	//		T = Cycle_path_helper(v,i,path) ;
	//		if(T)
	//			return ;
	//	}
	//}
	//bool Cycle_path_helper(int v,int i,int* path_help)
	//{
	//	path_help[i++] = v ;
	//	Edge<EdgeType> e ;
	//	for(e = this->FirstEdge(v); this->IsEdge(e); e = this->NextEdge(e))
	//	{
	//		if(e.end == v)
	//			return true;
	//		else
	//			Cycle_path_helper(e.end,i,path_help) ;
	//	}
	//	return false ;
	//}
};
